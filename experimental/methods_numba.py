...
def conv2d(input, weights, mode="constant"):
    ...
    kh, kw = weights.shape
    # dh, dw = np.floor(kh / 2, dtype=np.int16), np.floor(kw / 2, dtype=np.int16)
    dh, dw = kh // 2, kw // 2
    origin = np.zeros((h + dh * 2, w + dw * 2))
    origin[dh : h + dh, dw : w + dw] = input.copy()
    for i in range(h):
        for j in range(w):
            output[i, j] = np.sum(origin[i : i + kh, j : j + kw] * weights)
    return output
...
def canny_numba(
    ...
    image_smoothed = image_smoothed.astype(np.float32)
    ...
    # numba处理索引不行
    for i in range(len(strong[0])):
        edge[strong[0][i], strong[1][i]] = 255
    for i in range(len(uncertain[0])):
        edge[uncertain[0][i], uncertain[1][i]] = 100
...
if __name__ == "__main__":
    ...
    # conv2d(a, np.ones((3, 3)) / 9)
    edge, direction = canny_numba(img, high_threshold=100, low_threshold=50)
